...
## 2026-08-06 C01~C13 재검증 — **화면 표시 결함 발견**

수기 대조분 13건을 브라우저 자동 입력으로 다시 돌리면서 **공단 화면의 구성 항목까지**
대조했다. 금액 결과는 13건 전부 재현됐고 문서 기록값과도 같았다.

**그런데 ① 소득월액보험료에서 9건이 어긋났다.** 공단은 ① 칸에 **하한·상한을 적용한 값**을
넣어 `① + ③ = ④`가 성립하게 한다. 우리는 원값을 그대로 화면에 뿌려서
`0원 + 139,378원 = 159,530원`처럼 **표에서 덧셈이 안 맞았다.**

- 공식: **공단 ① = clamp(소득분 원값, 하한 20,160원, 상한 4,591,740원)**.
  C01~C25 **23건 전부** 일치.
- 조치: `PremiumBreakdown.incomePortionApplied` 추가(원값은 `incomePortion`으로 유지).
  **화면에는 반드시 `incomePortionApplied`를 쓴다.** 지역보험료 계산기와
  임의계속 비교 양쪽 다 반영했다.
- 상한이 걸린 사례는 공단 화면에서도 `① + ③ ≠ ④`다(①과 ④에 각각 상한 적용).
  기존 상한 안내 문구가 설명한다.
- **교훈: 합계만 대조하면 화면 결함을 못 잡는다.** 다음 대조도 구성 항목까지 본다.

## 2026-08-06 지역보험료 대조 확대 (A안 4-3 완료) — **전월세 규칙 결함 발견**

- 지역보험료 공단 모의계산 대조를 **13건 → 24건**(C14~C25)으로 늘렸다.
  금액뿐 아니라 **재산점수와 구성 보험료까지** 대조했다.
- **설계 중 발견:** C01~C13은 소득만 또는 재산만 넣은 사례였다. **소득분 + 재산분
  합산 지점이 한 번도 대조되지 않았다.** C14·C17·C21이 메웠고 전부 일치.
- C18에서 공단 화면이 소득 0원인데도 소득월액보험료를 20,160원으로 표시 →
  **재산만 있어도 소득 하한이 붙는다**는 가정을 화면으로 확인.

### 전월세 반영 규칙 (중요)

**주택 또는 건물을 보유하면 금액과 무관하게 전월세를 재산에 반영하지 않는다.
토지·선박·항공기만 있으면 합산한다.** 공단 실측:

| 입력 | 공단 재산점수 |
|---|---:|
| 주택 2억 + 전세 2억 | 439 (= 주택 2억 단독) |
| 주택 **1만원** + 전세 2억 | **0** |
| 건물 2억 + 전세 2억 | 439 |
| **토지** 2억 + 전세 2억 | **535** (합산됨) |

- 화면 결과는 틀리지 않았다. 「주택·건물을 소유하지 않고 임차 중입니다」 체크박스가
  규칙을 지키고 있었다. 대조가 어긋난 건 그 체크박스를 우회해 라이브러리에
  합산값을 직접 넣었기 때문이다.
- **그러나 규칙이 체크박스 라벨에만 있고 모델에 없었다.** 호출처가 한 곳이라
  결과는 맞았지만 문구를 고치거나 호출처가 늘면 조용히 깨지는 구조였다.
- 조치: `propertyAmountFor()`를 `src/lib/constants/property-score-table.ts`에 추가.
  **재산금액을 직접 더하지 말고 이 함수를 쓴다.** 공단 실측값(439·535·0점) 기반
  회귀 검사를 `src/lib/premium/regional.test.ts`에 추가했다.
- 공개 표에 C24·C25로 **불일치를 발견하고 원인을 찾은 과정 그대로** 남겼다.
  3-4에서 만든 표의 존재 이유("불일치도 숨기지 않는다")의 첫 시험이었다.

### 공단 모의계산 화면에서 확인한 것 (다음 대조 때 참고)

- 재산 입력이 **주택/건물/토지/선박/항공기로 분리**되어 있다. "재산세 과세표준"이라는
  단일 칸은 없다. 도움말에 "6월 1일 기준 재산세 과세표준금액"이라고 적혀 있다.
- 전월세는 **전세(보증금)/월세(보증금)/월세** 3칸. "부과 전월세"는 공단 직권 부과
  금액이라 입력 불가(비활성).
- **우리 모델에 없는 입력 항목 3개** — 미검토 상태다:
  **분리과세 주택임대소득** / **주택금융부채 공제(주담대 잔액)** / **농어촌 지역 거주
  22% 경감**. 셋 다 보험료를 실제로 바꾼다.

## 2026-08-06 사적연금 합산 여부 확인 (A안 4-2 완료)

- **중단 조건 2번은 발동하지 않는다. 판정 로직은 그대로 두었다.**
  사적연금(연금저축·IRP)은 피부양자 합산소득에 들어가지 않는 것이 맞다.
  세금핏의 "공적+사적 모두 포함"은 공단 부과 실무와 맞지 않는다.
- **그러나 우리도 근거를 잘못 말하고 있었다.** 시행령 제41조제1항제5호는 연금소득을
  「소득세법 제20조의3에 따른 소득」이라고만 하고 **사적연금(제20조의3제1항제2호)을
  문언상 배제하지 않는다.** 조문만 읽으면 포함되는 것처럼 보인다.
- **실제로 빠지는 지점은 부과 자료의 범위다.** 연금소득 자료는 국세청이 아니라
  5대 공적연금 지급기관이 제공하고, 그들은 연금저축·IRP 자료를 갖고 있지 않다.
  시행령 제41조제3항이 연금소득에만 다른 반영시기(전년도)를 정한 것이 그 흔적이다.
- **이 차이가 실무적으로 중요한 이유:** 근거가 법령이면 개정 전까지 안정적이지만,
  근거가 자료 연계라면 **법 개정 없이 결론이 바뀔 수 있다.** 근거를 법령으로 적어두면
  그 변화를 놓친다. 그래서 코드 주석·가이드·검증기록을 전부 이 구조로 고쳤다.
- 관련: `src/lib/dependent/judge.ts`(`sumIncome` 주석),
  `src/app/health-insurance/guides/pension-impact/page.tsx`(「사적연금이 빠지는 진짜 이유」 절),
  `docs/03-검증기록.md`, `src/lib/routes.ts`(`lastModified` → 2026-08-06).
- **남은 한계:** 공단의 법령 해석 명의 문서로는 확인하지 못했다. 근거는 시행령 원문과
  공단 발행 웹진(외부 필자 기고)이다. **그 웹진은 2023년 9월 기고라 재산공제 5,000만원·
  자동차 보험료 같은 낡은 수치가 섞여 있으므로 연금 구분 서술만 인용한다.**
