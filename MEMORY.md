...
## 2026-08-06 농어촌 지역 거주 경감 구현

- 공단 화면에는 있는데 우리가 안 받던 입력 3개 중 **첫 번째를 구현했다.**
  조사 결과는 `reports/미구현-입력항목-규칙조사-2026-08.md`.
- **계산 순서가 중요하다.** 경감은 건강보험료를 직접 깎지 않는다:

  ```
  건강보험료 = clamp(소득분+재산분, 하한, 상한)   ← 경감 전 값 유지
  경감액     = floor10(건강보험료 × 22%)
  장기요양   = floor10((건강보험료 − 경감액) × 장기요양율 ÷ 건강보험료율)
  합계       = 건강보험료 − 경감액 + 장기요양
  ```

  **장기요양을 경감 전 기준으로 계산하면 과대 계산된다.** 실측 C26에서
  20,960원 → 16,350원으로 줄었다.
- **하한에도 경감이 붙는다.** 소득·재산 0원 → 건강 20,160원, 경감 4,430원, 최종 17,790원.
- 요건: 읍·면 거주 + **사업소득 500만원 이하**. 초과 시 농어업인 등록자 필요.
  주민등록 주소지 일괄 적용(신청 불필요). 근거는 `BASIS.RURAL_REDUCTION`.

### **공단 모의계산기와 의도적으로 다르게 계산하는 첫 사례 (C28)**

- **공단 모의계산기는 사업소득 500만원 조건을 검사하지 않는다.** 사업소득 600만원에
  체크만 하면 22%를 적용한다(2026-08-06 실측).
- **기준체크는 제도 기준으로 판정한다.** 계산기를 따라 하면 실제보다 적은 금액을
  보여주게 된다. 대신 경감 미적용 사유와 "공단 계산기는 이 조건을 확인하지 않는다"를
  화면에 함께 표시한다.
- 사례 표에서 **`공단 결과`와 우리 결과가 다른 유일한 행**이다. 오류가 아니라 의도한
  차이이므로 숨기지 않는다.

### 구현하지 않는 경감 (한계로 표시할 것)

65세·70세 이상, 한부모, 장애인, 만성질환, 55세 이상 여자 단독세대(10~30%),
섬·벽지(50%), 농어업인(0~28%). **공단 모의계산기 자신이 반영하지 않는다** —
화면 상단에 명시돼 있다. 우리 목표가 모의계산과의 일치이므로 범위 밖이다.

### 남은 미구현 입력 2개

- **주택금융부채 공제** — 요건은 확정(1세대 1주택/무주택, 공시가격 5억 이하,
  전후 3개월 이내 대출, 신청 필요). **계산식 미확정**: 평가율이 자료마다 60% vs 70%로
  갈리고, 인용된 시행령 별표4가 기본공제 5,000만원·208.4원인 **옛 버전**이라
  공제 한도도 현행이라 볼 수 없다. **시행령 별표4 현행 원문 확보가 선행 조건.**
- **분리과세 주택임대소득** — 반영률 미확인. 공단 모의계산 2건이면 나온다.

> 입력 구조 결론: 재산을 5칸으로 쪼갤 필요는 없다. 주담대 공제를 구현할 때
> 「무주택 / 1세대 1주택 / 그 외」 3분류가 필요하고, 그게 지금의
> 「주택·건물 미소유」 체크박스를 대체하면서 전월세 억제 규칙까지 함께 표현한다.
