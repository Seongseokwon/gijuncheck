...
- P2-1 지역보험료 계산기 공식 모의계산 13건 대조 → 이후 금융소득 기준 수정으로 **재대조 전** (`VERIFIED_AGAINST_NHIS = false`)
...
0. **[A안 3-2 · 최우선] 지역보험료 금융소득 재대조를 끝내고 `VERIFIED_AGAINST_NHIS`를
   `true`로 되돌린다.** 포지셔닝이 검증 깊이인데 사이트에 `재대조 전`이 남아 있다.
0-1. **[A안 3-3] 배우자 동반(부부 합산) 판정 구현.** 시행규칙 별표 1의2에서 법적 근거부터
   확인한다 — 경쟁사 화면을 근거로 삼지 않는다.
