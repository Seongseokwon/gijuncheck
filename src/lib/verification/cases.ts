 ...
 * 2026-08-06 추가 대조.
 *
 * C01~C13은 **소득만 또는 재산만** 넣은 사례여서, 실제 산식의 핵심인
 * 소득분 + 재산분 합산 지점이 한 번도 공단과 대조되지 않았다. C14·C17·C21이
 * 그 구멍을 메운다.
 */
const REGIONAL_CASES_2026_08_06: readonly VerificationCase[] = [
  [
    'C14',
    '사업소득 등 1,200만원 + 주택 과세표준 3억원',
    '221,560원',
    '소득과 재산을 동시에 넣은 첫 대조. 재산점수(586점)와 구성 보험료까지 공단 화면과 같았다',
  ],
  ['C15', '근로소득 3,000만원', '101,670원', '근로소득 단독 50% 반영'],
  ['C16', '연금소득 2,400만원', '81,340원', '연금소득 단독 50% 반영. 은퇴자에게 가장 흔한 형태'],
  [
    'C17',
    '연금소득 2,400만원 + 주택 과세표준 5억원',
    '262,480원',
    '연금 수령자가 집을 가진 경우. 재산점수 757점',
  ],
  [
    'C18',
    '주택 과세표준 10억원 (소득 0원)',
    '262,330원',
    '소득이 0원이어도 공단 화면이 소득월액보험료를 하한 20,160원으로 표시했다. 재산만 있어도 소득 하한이 붙는다는 계산 가정을 화면으로 확인',
  ],
  [
    'C20',
    '월세 보증금 3억원 + 월세 100만원',
    '28,070원',
    '(보증금 + 월세×40) × 30% 환산식을 큰 월세에서 확인. 재산점수 22점',
  ],
  [
    'C21',
    '사업소득 등 20억원 + 주택 과세표준 20억원',
    '5,195,110원',
    '재산이 있어도 건강보험료 상한 4,591,740원에서 멈춘다. 소득만으로 상한에 닿은 C11과 합계가 같다',
  ],
  ['C22', '사업소득 등 336만원', '22,800원', '하한이 적용되는 마지막 구간'],
  [
    'C23',
    '사업소득 등 340만원',
    '23,040원',
    'C22와 4만원 차이로 하한을 벗어난다. 두 사례가 같은 값이면 하한 적용 방식이 다르다는 뜻이다',
  ],
].map(([id, input, amount, note]) => ({
  id,
  input,
...
/**
 * 전월세 반영 규칙 — 대조 중 불일치가 나서 원인을 찾은 사례.
 *
 * **이 두 건이 이 표에서 가장 값어치 있는 기록이다.** 처음에 「주택 과세표준 2억 +
 * 전세보증금 2억」을 넣고 둘을 더해 계산했더니 공단과 22,980원이 어긋났다.
 * 조건을 바꿔가며 다섯 번 더 돌려 원인을 찾았다.
 */
const RENT_RULE_CASES: readonly VerificationCase[] = [
  {
    id: 'C24',
    input: '주택 과세표준 2억원 + 전세보증금 2억원',
    expected: '127,840원 (재산점수 439점)',
    actual: '127,840원',
    diff: '0원',
    result: 'match',
    checkedOn: '2026-08-06',
    note: '주택을 소유하면 전월세는 재산에 반영되지 않는다. 주택 2억원만 넣은 결과와 값이 같았다. 주택 금액을 1만원으로 낮춰도 전세 2억원이 그대로 빠졌으므로, 기준은 금액이 아니라 보유 여부다. 처음에는 둘을 더해 150,820원으로 계산해 어긋났고, 그 차이에서 이 규칙을 찾았다',
  },
  {
    id: 'C25',
    input: '토지 과세표준 2억원 + 전세보증금 2억원',
    expected: '150,820원 (재산점수 535점)',
    actual: '150,820원',
    diff: '0원',
    result: 'match',
    checkedOn: '2026-08-06',
    note: '반대로 토지는 전월세와 합산된다. 주택·건물이 아니면 임차인으로 보기 때문이다. C24와 입력 금액이 같은데 결과가 다르다는 점이 이 규칙의 핵심이다',
  },
];
...
export const VERIFICATION_CASE_GROUPS: readonly VerificationCaseGroup[] = [
  {
    ...
    tier: 'nhis-simulator',
    summary:
      '공단 모의계산기에 같은 값을 입력하고 화면 금액과 비교했습니다. 금액뿐 아니라 재산점수와 구성 보험료까지 대조했습니다. 공단 입력란이 만원 단위 정수만 받으므로 1원 단위 경계는 이 표가 아니라 코드 단위 테스트에서 확인합니다.',
