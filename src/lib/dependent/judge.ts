 ...
 *
 * ## 사적연금 제외의 근거 — 2026-08-06 재확인
 *
 * **법령 문언만으로는 이 결론이 나오지 않는다.** 이전 주석은 사적연금 제외를
 * 법령이 정한 사실처럼 적었는데, 실제 구조는 다음과 같다.
 *
 * 1. 시행규칙 별표 1의2 → 소득요건을 **시행령 제41조제1항 각 호의 소득 합계**로 정의
 * 2. 시행령 제41조제1항제5호 → 연금소득은 「소득세법」 제20조의3에 따른 소득.
 *    **제20조의3제1항제2호(연금계좌 인출 = 사적연금)를 문언상 배제하지 않는다.**
 *    단서는 공적연금에 제20조의3제2항을 적용하지 않는다(= 전액 본다)는 내용뿐이다.
 * 3. 실제로 사적연금이 빠지는 지점은 **자료 출처**다. 시행령 제41조제3항제1호 단서가
 *    연금소득 자료만 전년도(다른 소득은 전전년도)로 정하는데, 이는 연금소득 자료를
 *    국세청이 아니라 5대 공적연금 지급기관에서 받기 때문이다. 공적연금 지급기관은
 *    사적연금 자료를 갖고 있지 않다.
 * 4. 공단 웹진(2023-09)도 "연금소득에는 공적연금은 포함되고 사적연금은 포함되지
 *    않는다"고 명시한다.
 *
 * 즉 **결론은 맞지만 근거는 "법령이 배제한다"가 아니라 "부과 자료에 들어오지
 * 않는다"이다.** 이 구분이 중요한 이유는, 자료 연계 방식이 바뀌면 법령 개정 없이도
 * 결론이 바뀔 수 있기 때문이다. 근거를 법령 문언으로 적어두면 그 변화를 놓친다.
 *
 * 상세는 `docs/03-검증기록.md`의 「2026-08-06 사적연금 합산 여부 확인」.
 */
